...
import { 
  ...
  GripVertical, Clock, Map, Sparkles, Save, Edit2, 
  Check, Navigation, ChevronDown, ChevronUp, ChevronLeft, ChevronRight,
  PanelLeftClose, PanelLeftOpen, Heart, Bookmark, MoreVertical, Trash2, FolderOpen, List
} from 'lucide-react';
    ...
    >
      ...
      >
        <div className="w-12 h-12 rounded-md overflow-hidden shrink-0 bg-gray-100">
          <img src={stop.image} alt="" className="w-full h-full object-cover" />
        </div>
        <div className="flex-1 min-w-0">
          <h4 className="font-bold text-slate-800 text-sm truncate hover:text-[#FF6B35] transition-colors">{stop.name}</h4>
          <p className="text-xs text-gray-500">Est. 1 hour</p>
        </div>
      </div>
      <button onClick={() => removeStop(stop.id)} className="text-gray-400 hover:text-red-500 p-1">
        <X className="w-4 h-4" />
      </button>
    </div>
  );
...
export const Planner = () => {
  ...
  const [showMiniItinerary, setShowMiniItinerary] = useState(false);
  
  // Tour Menu state
  const [showTourMenu, setShowTourMenu] = useState(false);

  const availableTags = ['Foodie', 'Nightlife', 'Cultural', 'Family Friendly', 'Budget', 'Luxury', 'Adventure', 'Coffee'];
  ...
  const [previousRestaurant, setPreviousRestaurant] = useState<Restaurant | null>(null);
  const [showSaved, setShowSaved] = useState(false);
  const [selectedTour, setSelectedTour] = useState<any | null>(null);
  const [previousTour, setPreviousTour] = useState<any | null>(null);
  
  // Initialize favorites with all MOCK_RESTAURANTS (same as Profile page)
  const [favoriteRestaurants, setFavoriteRestaurants] = useState<Restaurant[]>(MOCK_RESTAURANTS);
  // Initialize saved tours with all MOCK_TOURS (same as Profile page)
  const [savedTours, setSavedTours] = useState<any[]>(MOCK_TOURS);
  
  // Track which saved category is being viewed: null | 'favorites' | 'tours'
  const [savedCategory, setSavedCategory] = useState<'favorites' | 'tours' | null>(null);
  ...
  useEffect(() => {
    const storedTours = JSON.parse(localStorage.getItem('savedTours') || '[]');
    setMyTours(storedTours);
  }, []);
  
  // Track previous view for back navigation: 'search' | 'favorites' | 'tours' | 'saved-menu' | 'tour-detail' | 'my-tours' | 'current-itinerary' | 'tour-menu'
  const [previousView, setPreviousView] = useState<'search' | 'favorites' | 'tours' | 'saved-menu' | 'tour-detail' | 'my-tours' | 'current-itinerary' | 'tour-menu'>('search');

  const districts = ['District 1', 'District 2', 'District 3', 'District 4'];
  const priceRanges = ['$', '$$', '$$$', '$$$$'];
  const cuisines = ['Vietnamese', 'Phở', 'Bánh Mì', 'Coffee', 'Seafood', 'Fusion'];
  ...
  const loadTour = (tour: any) => {
    ...
    if (tour.stops && Array.isArray(tour.stops)) {
      ...
      tourRestaurants = MOCK_RESTAURANTS.filter(r => 
        r.tags.includes('Coffee') || r.name.toLowerCase().includes('coffee')
      ).slice(0, 3);
    } else if (tour.id === 't3') {
  ...
  useEffect(() => {
    if (location.state?.tour) {
      ...
      const matchingTour = MOCK_TOURS.find(t => t.id === tourFromState.id);
      if (matchingTour) {
        ...
        setPreviousView('search'); // Set to search since we came from outside
  ...
  const filteredRestaurants = MOCK_RESTAURANTS.filter(r => {
    if (selectedDistrict && !r.address.includes(selectedDistrict)) return false;
    if (selectedPrice && r.priceRange !== selectedPrice) return false;
    if (selectedCuisine && !r.tags.includes(selectedCuisine)) return false;
  ...
  const toggleRestaurantSelection = (restaurant: Restaurant) => {
    ...
    if (isSelected) {
      ...
      if (!showItinerary && tourStops.length === 0) {
        toast.success('Restaurant added to tour! Open itinerary to view.');
      }
    }
  };
  ...
  const handleSaveTour = () => {
    const tour = {
      ...
      createdAt: editingTourId 
        ? (myTours.find(t => t.id === editingTourId)?.createdAt || new Date().toISOString()) 
    ...
    if (editingTourId) {
      ...
      newSavedTours = myTours.map(t => t.id === editingTourId ? tour : t);
      
      // Update in Saved Tours if present there too
      if (savedTours.find(t => t.id === editingTourId)) {
          setSavedTours(savedTours.map(t => t.id === editingTourId ? tour : t));
      }
    ...
    localStorage.setItem('savedTours', JSON.stringify(newSavedTours));
  ...
  const handleDeleteMyTour = (id: string) => {
    ...
    setMyTours(newTours);
    localStorage.setItem('savedTours', JSON.stringify(newTours));
  ...
  const handleMapDotClick = (restaurant: Restaurant) => {
    // Track that we came from search results or saved views
    if (showItinerary) {
  ...
  const handleTourClick = (tour: any) => {
    // Track that we came from saved tours
    setPreviousView('tours');
    setSelectedTour(tour);
    setShowSaved(false); // Hide the saved view when viewing tour detail
    setShowTourMenu(false);
    setSavedCategory(null); // Clear saved category when viewing tour detail
    setSelectedRestaurant(null);
    setShowItinerary(false);
  };
        ...
        >
          ...
          {isPanelCollapsed && (
            <div className="flex flex-col items-center py-6 gap-6 h-full">
              ...
              {/* Active Itinerary Indicator - Optional now since it's in Tour menu, but maybe keep as shortcut? User didn't ask to remove */}
              {tourStops.length > 0 && (
                <button
                  onClick={() => {
                    setIsPanelCollapsed(false);
          ...
          {!isPanelCollapsed && (
            <>
              ...
              {/* Search Header - Only show when NOT in saved view AND NOT in my tours view AND NOT viewing tour detail AND NOT in Tour Menu */}
              {!showSaved && !showMyTours && !selectedTour && !showTourMenu && !showItinerary && (
                <>
                  <div className="p-4 border-b border-gray-200 relative z-20 bg-white">
                  ...
                  {/* Filters Overlay */}
                  {showFilters && (
                    <div className="absolute top-[109px] left-0 right-0 bg-white shadow-lg border-b border-gray-200 z-30 animate-in slide-in-from-top-4 duration-200">
                      <div className="space-y-3 p-4 bg-gray-50 m-4 rounded-lg max-h-[calc(100vh-300px)] overflow-y-auto">
                        <div>
                    ...
                    {savedCategory === 'favorites' ? (
                      ...
                      <>
                        ...
                        <div className="mb-6">
                          ...
                          <p className="text-sm text-gray-500 mt-1">{favoriteRestaurants.length} place{favoriteRestaurants.length !== 1 ? 's' : ''}</p>
                        </div>
                  ...
                  <div className="p-4">
                    ...
                    <div className="mb-6">
                      ...
                      <p className="text-sm text-gray-500 mt-1">
                         You have {myTours.length} created tour{myTours.length !== 1 ? 's' : ''}
                      </p>
                    </div>
                    ...
                    {myTours.length === 0 ? (
                      ...
                      <div className="space-y-3">
                        {[...myTours].sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()).map((tour) => (
                          <div
                            key={tour.id}
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-4 flex-wrap mb-4">
                          ...
                          <div className="flex items-center gap-1 text-gray-600">
                            ...
                            <span>{selectedTour.duration || `${((Array.isArray(selectedTour.stops) ? selectedTour.stops.length : (typeof selectedTour.stops === 'number' ? selectedTour.stops : 0)) * 1.5).toFixed(1)} hrs`}</span>
                          </div>
                          <span className="text-gray-400">•</span>
                          <div className="flex items-center gap-1 text-gray-600">
                            <MapPin className="w-5 h-5" />
                            <span>{Array.isArray(selectedTour.stops) ? selectedTour.stops.length : (typeof selectedTour.stops === 'number' ? selectedTour.stops : 0)} stops</span>
                          </div>
                          {selectedTour.distance && (
                            <>
                              <span className="text-gray-400">•</span>
                              <span className="text-gray-600">{selectedTour.distance}</span>
                            </>
                          )}
                        </div>
                        ...
                        {/* Action Buttons */}
                        {myTours.find(t => t.id === selectedTour.id) ? (
                          <div className="flex flex-col gap-3 mb-4">
                            <button
                              onClick={() => handleEditMyTour(selectedTour)}
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                ...
                                <button
                                    onClick={() => {
                                        if (savedTours.find(t => t.id === selectedTour.id)) {
                                    ...
                                    className={`flex items-center justify-center gap-2 px-4 py-3 rounded-lg font-medium transition-colors ${
                                        savedTours.find(t => t.id === selectedTour.id)
                                            ? 'bg-pink-100 text-pink-600 border border-pink-200 hover:bg-pink-200'
                                            : 'bg-white text-gray-600 border border-gray-300 hover:bg-gray-50'
                                    }`}
                                    title={savedTours.find(t => t.id === selectedTour.id) ? "Remove from saved" : "Save to bookmarks"}
                                >
                                    <Bookmark className={`w-4 h-4 ${savedTours.find(t => t.id === selectedTour.id) ? "fill-current" : ""}`} /> 
                                    {savedTours.find(t => t.id === selectedTour.id) ? "Saved" : "Save"}
                                </button>
                            </div>
                            ...
                            <button
                              onClick={() => {
                                if (window.confirm('Are you sure you want to delete this tour?')) {
                                  handleDeleteMyTour(selectedTour.id);
                                  setShowMyTours(true);
                                  setSelectedTour(null);
                                }
                              }}
                          ...
                          <div className="flex flex-col gap-3 mb-4">
                            <div className="grid grid-cols-2 gap-3">
                                ...
                                <button
                                    onClick={() => {
                                        if (savedTours.find(t => t.id === selectedTour.id)) {
                                            removeSavedTour(selectedTour.id);
                                        } else {
                                            setSavedTours([...savedTours, selectedTour]);
                      ...
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <button
                          ...
                          className={`px-4 py-2 rounded-lg font-medium transition-colors flex-shrink-0 ${
                            tourStops.find(s => s.id === selectedRestaurant.id)
                              ? 'bg-[#2E86AB] text-white'
                              : 'bg-[#FF6B35] text-white hover:bg-[#e55a2b]'
                          }`}
                        >
                          {tourStops.find(s => s.id === selectedRestaurant.id) ? '✓ Added' : '+ Add'}
                        </button>
                      </div>
                      ...
                      <div className="mb-4">
                                    ...
                                    <button
                                      ...
                                      onClick={() => {
                                        if (tourTags.includes(tag)) {
                                          setTourTags(tourTags.filter(t => t !== tag));
                                        } else {
                                          setTourTags([...tourTags, tag]);
                                        }
                                      }}
                            ...
                            <div>
                              <div className="flex items-start gap-2 group mb-2">
                                <div className="flex-1">
                                  ...
                                  {tourDescription && (
                                    <p className="text-sm text-gray-600 mt-1">{tourDescription}</p>
                                  )}
                                  {tourTags.length > 0 && (
                        ...
                        <div className="mt-2">
                          <textarea
                            ...
                            onChange={(e) => setTourDescription(e.target.value)}
                            placeholder="Add a description for your tour..."
                            className="w-full p-2 bg-white border border-gray-200 rounded-lg text-sm text-gray-600 focus:border-[#FF6B35] outline-none resize-none"
                            rows={2}
                          />
                        </div>
                        ...
                        {tourStops.map((stop, index) => (
                          <div key={stop.id} className="relative flex items-start gap-3">
                            ...
                            <DraggableStop 
                              ...
                              onStopClick={handleRestaurantClick}
                            />
                          </div>
                        </div>
                      ))}
                  ...
                  <>
                        ...
                        >
                          <div className="flex gap-4">
                            <Link 
                              ...
                              className="w-32 h-32 flex-shrink-0 rounded-lg overflow-hidden"
                            >
                              <img
                    ...
                    {filteredRestaurants.length === 0 && (
                      <div className="flex flex-col items-center justify-center h-full text-gray-500 p-8">
                        ...
                        <p className="text-sm text-center mt-2">Try adjusting your filters or search query</p>
                      </div>
                    )}
                  </>
                )}
