...
export const DishDetailPanel = ({
  ...
  const priceRange =
    selectedDish.minPrice === selectedDish.maxPrice
      ? formatVND(selectedDish.minPrice)
      : `${formatVND(selectedDish.minPrice)} - ${formatVND(selectedDish.maxPrice)}`;
      ...
      <div className="space-y-6">
        <div className="w-full h-56 rounded-xl overflow-hidden bg-gray-100 relative">
          ...
          <div className="absolute bottom-4 left-4 right-4 text-white">
            <h2 className="text-3xl font-bold mb-1 break-words">{selectedDish.name}</h2>
            <p className="opacity-90 font-medium">{priceRange}</p>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {selectedDish.tags?.map((tag: string, idx: number) => (
              <span
                key={idx}
        ...
        <div>
          <h3 className="font-bold text-gray-900 mb-4 flex items-center gap-2">
            ...
            Available at {selectedDish.restaurants.length} places
          </h3>
          <div className="space-y-3">
            {selectedDish.restaurants.map((restaurant: Restaurant) => {
              const specificDish = restaurant.dishes.find(
                (dish) => dish.name.trim() === selectedDish.name,
              );
                        ...
                        <div className="flex flex-col items-end gap-1 shrink-0">
                          {specificDish && (
                            <span className="text-sm font-bold text-emerald-600">
                              {formatVND(parsePrice(specificDish.price))}
                            </span>
                          )}
